...
% input dir
input_dir = '\\surrey.ac.uk\personal\HS216\tm00529\MATLAB\ML_output\v4dp3_T2p1V2_75_ANN5_050\';
...
for i = 1:flh
    ...
    if isequal(1,regexp(fname,'[^ . _]\w*[.]JPG'))
            ...
            for sj = 1:tile:wd
                if (si+stile <= ht && sj+stile <= wd)
                    ...
                    if (predict && actual) % TP
                        % draw green tile boarder
                        Image(si:si+stile,sj:sj+tbdr,2) = 255;
                        Image(si:si+tbdr,sj:sj+stile,2) = 255;
                        Image(si:si+stile,sj+stile-tbdr:sj+stile,2) = 255;
                        Image(si+stile-tbdr:si+stile,sj:sj+stile,2) = 255;
                        % black out colours other than tile boarder
                        Image(si:si+stile,sj:sj+tbdr,1:2:3) = 0;
                        Image(si:si+tbdr,sj:sj+stile,1:2:3) = 0;
                        Image(si:si+stile,sj+stile-tbdr:sj+stile,1:2:3) = 0;
                        Image(si+stile-tbdr:si+stile,sj:sj+stile,1:2:3) = 0;
                    elseif (~predict && actual) % FN
                    ...
                    elseif (predict && ~actual) % FP
                        % draw yellow tile boarder
                        Image(si:si+stile,sj:sj+tbdr,1:2) = 255;
                        Image(si:si+tbdr,sj:sj+stile,1:2) = 255;
                        Image(si:si+stile,sj+stile-tbdr:sj+stile,1:2) = 255;
                        Image(si+stile-tbdr:si+stile,sj:sj+stile,1:2) = 255;
                        % black out colours other than tile boarder
                        Image(si:si+stile,sj:sj+tbdr,3) = 0;
                        Image(si:si+tbdr,sj:sj+stile,3) = 0;
                        Image(si:si+stile,sj+stile-tbdr:sj+stile,3) = 0;
                        Image(si+stile-tbdr:si+stile,sj:sj+stile,3) = 0;
                    end;
                end;
            end;
        end;
        ...
        imwrite(small,strcat(output_dir,iname,'_ClassificationComp','.jpg'));
    end;
end;
%figure, imshow(small);
